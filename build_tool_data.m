function [ncon, X, Y, F, dzero, n_nodes, n_element, E, v, t, NDU] = build_tool_data(grid_density, varargin)
    ...
    %
    % Takes input of grid density, for quality of mesh used, and varargin, which is all the data from the input fields
    default_nodes = [
        0       0;
        0.0005  0;
        0.006   0;
    ...
    if nargin >= 2 && ~isempty(varargin)
        ...
        %override by user inputs
        E = 210e9;
        v = 0.3;
        t = 1;
    end
    % Generate mesh polygon vertices using passed in grid density
    ...
    if isempty(fixed_nodes)
        ...
        % Get the y-coordinates of the specified fixed vertices and fix all nodes at that y
    ...
    % Assemble constrained DOF for each fixed node
    dzero = [];
    for i = fixed_nodes
