...
%
% Input arguments:
% filename - output Excel filename (string); default 'FEA_results.xlsx'
% app      - app object containing model data and UI handles
% Export all model inputs and solved results onto one clean Excel sheet
if nargin < 1 || isempty(filename)
    % Set default filename when not provided
...
% Flatten and copy app data into local arrays
X     = app.X(:);
Y     = app.Y(:);
ncon  = app.ncon;
F     = app.F(:);
...
% Compute and write basic statistics for key fields
